// matmul_save_factors_nostg: VARIANT with NO SMEM staging in the epilogue.
//
// ── STATUS: tested, SIGNIFICANT REGRESSION (~300 µs slower). ──────
// At M=11136 K=3584 N=14336 (B200), bench_stable.py:
//     staging  (PERS NS=7) median = 1.982 ms
//     no-staging (PERS NS=7) median = 2.277 ms   (+295 µs)
//
// Diagnosis: uncoalesced GMEM writes (each warp's 32 lanes target 32
// different rows = 32 different cache lines per int4 store) overwhelm
// the L2's ability to coalesce partial cache lines.  HBM bandwidth is
// wasted on partial-line write-amplification.
//
// Implication: the SMEM staging step in the production kernel is
// doing real work by re-arranging Phase 1's row-per-lane pattern
// into Phase 2's coalesced-by-row pattern.  Path B must keep SMEM
// staging — the design is fixed at "staging in dedicated SMEM region,
// NS=4 ring, double-buffered TMEM, warp role split."
//
// EXPERIMENT to test whether uncoalesced GMEM writes from registers are
// tolerable at this shape — the answer dictates which Path B variant
// to build (with-staging vs without-staging).
//
// Architecture vs `_matmul_save_factors.cu`:
//   - Phase 1: TMEM read → activation in registers → DIRECT GMEM stores
//     (uncoalesced, since each warp's 32 lanes own DIFFERENT rows).
//   - Phase 2: ELIMINATED.  No SMEM staging buffer.
//   - End-of-tile: single __syncthreads (no inter-phase sync needed).
//
// Coalescing characterization:
//   - Each warp's 32 lanes target 32 DIFFERENT GMEM rows simultaneously
//     → 32 cache lines per int4 store instruction.
//   - L2 (132 MB) buffers and may combine partial cache lines before
//     draining to HBM.  If L2 holds them long enough, HBM bandwidth is
//     not wasted; if it evicts early, partial cache line writes amplify
//     HBM traffic.
//
// This is a SERIAL persistent kernel (no K-loop/epilogue overlap yet)
// — the experiment is just about the no-staging epilogue, with all
// else equal to the production save_factors kernel.
//
// ── inherited save_factors header ──────────────────────────────────
//
...
#define LOAD_TILE(slot_, k0_)                                                          \
    ...
    //
    // Single-phase epilogue: TMEM → registers → GMEM directly.  No SMEM staging.
    //
    // Work split: 4 row-warps × 2 col-warps = all 8 warps.
    //   row_warp ∈ {0..3} : 32-row strip of TMEM
    //   col_warp ∈ {0..1} : left-half or right-half of EACH chunk (BN_HALF/2 cols each)
    // Each lane ends up writing 32 cols × 3 outputs (out, factor_left,
    // factor_gate) to its OWN row of GMEM — uncoalesced across the warp
    // (32 lanes → 32 different cache lines per int4 store).
    ...
    // GMEM row + col bases for this lane's writes.
    const int gr            = off_m + my_row;
    const int out_n_base    = bid_n * BN_HALF;        // OUT col base
    const int fac_n_base    = off_n;                   // FAC col base (chunked)
    __nv_bfloat16* const out_row_ptr = &OUT_ptr[(size_t)gr * N_HALF + out_n_base];
    __nv_bfloat16* const fac_row_ptr = &FAC_ptr[(size_t)gr * N      + fac_n_base];

    {
        #pragma unroll
        for (int sub = 0; sub < COL_PER_WARP; sub += LD_X) {
            const int n_left = col_in_chunk + sub;
            // Read left chunk + gate chunk.
            ...
            // DIRECT GMEM stores (uncoalesced across warp lanes — lanes
            // own different rows).  Each lane writes 4 int4 chunks per
            // output for this LD_X tile.
            #pragma unroll
            for (int j = 0; j < LD_X / 8; j++) {
                *reinterpret_cast<int4*>(&out_row_ptr[n_left + j * 8]) =
                    *reinterpret_cast<int4*>(&out_pack[j * 4]);
                *reinterpret_cast<int4*>(&fac_row_ptr[n_left + j * 8]) =
                    *reinterpret_cast<int4*>(&fl_pack[j * 4]);
                *reinterpret_cast<int4*>(&fac_row_ptr[BN_HALF + n_left + j * 8]) =
                    *reinterpret_cast<int4*>(&fg_pack[j * 4]);
            }
        }
    }

    // End-of-tile barrier: ensure all warps have completed TMEM reads
    // before tile T+1's MMA overwrites TMEM.  GMEM stores are async to
    // L1/L2 and don't need explicit drain here (next tile's K-loop won't
    // wait on them; they retire in the background).
    __syncthreads();
...
void matmul_save_factors_nostg_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(         \
...
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                \
__launch_bounds__(256, LB_MIN_BLOCKS)                                                  \
void matmul_save_factors_nostg_pers_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(    \
    const __grid_constant__ CUtensorMap A_tmap,                                        \
    const __grid_constant__ CUtensorMap B_tmap,                                        \
    __nv_bfloat16* OUT_ptr, __nv_bfloat16* FAC_ptr,                                    \
    int M, int N_HALF, int K)                                                          \
