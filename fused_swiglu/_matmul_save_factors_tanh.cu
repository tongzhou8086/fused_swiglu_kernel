...
#define LOAD_TILE(slot_, k0_)                                                          \
    ...
    // Phase 1: all 8 warps read TMEM (4 row × 2 col split), fuse, stage to SMEM.
    {
        ...
        for (int sub = 0; sub < COL_PER_WARP; sub += LD_X) {
            ...
            //
            // tanh-form sigmoid: 0.5 + 0.5 * tanhf(g/2).  One SFU op
            // (MUFU.TANH) instead of the divide-form's two (MUFU.EX2 +
            // MUFU.RCP).  Halves SFU pressure in the activation epilogue.
            ...
            for (int i = 0; i < LD_X / 2; i++) {
                ...
                const float sig0  = 0.5f + 0.5f * tanhf(g0 * 0.5f);
                const float sig1  = 0.5f + 0.5f * tanhf(g1 * 0.5f);
...
void matmul_save_factors_tanh_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(          \
...
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                \
__launch_bounds__(256, LB_MIN_BLOCKS)                                                  \
void matmul_save_factors_tanh_pers_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(     \
    const __grid_constant__ CUtensorMap A_tmap,                                        \
    const __grid_constant__ CUtensorMap B_tmap,                                        \
    __nv_bfloat16* OUT_ptr, __nv_bfloat16* FAC_ptr,                                    \
    int M, int N_HALF, int K)                                                          \
