// matmul_save_factors_b_nostg: Path B + NO SMEM staging in epilogue.
//
// Same K-loop/epilogue overlap mechanism as _b.cu, but the epilogue
// writes DIRECTLY to GMEM from registers — no OUT_sh / FAC_sh staging.
// This frees the entire ~100 KB staging budget, letting the K-ring grow
// to NS=7 (224 KB) within the 228 KB cap.  At NS=7, num_k_iters/NS = 8
// (even) so phase parity stays consistent across tile boundaries.
//
// Tradeoff: GMEM stores from regs are stride-32 (each lane owns one row,
// writing 32 bf16 = 64 bytes); the hardware sees 32 distinct addresses
// per warp store.  We're betting that
//   - the deeper K-pipeline (NS=7 vs NS=4) helps MMA stay busier, AND
//   - the lost Phase 2 bar.sync + lost OUT_sh/FAC_sh writes/reads net
//     out against the worse GMEM access pattern.
//
// One bar.sync per tile disappears (no cross-warp SMEM dataflow), and
// the epi_done arrive moves to AFTER all TMEM reads.
//
// ── original Path B header ──
...
__device__ __forceinline__ void matmul_save_factors_impl(
    ...
    // ── SMEM layout: K-ring ONLY (no epi staging) ──────────────────────
    //   [0, RING_BYTES) : K-loop ring (NS × (A + B))
    // At NS=7 + BLOCK_M=BLOCK_N_LOCAL=128 + BK=64 → 224 KB, fits in 228 KB.
...
#define COMPUTE_TILE_OFFSETS(cid_, off_m_out, off_n_out, off_n_local_out, bid_n_out)    \
    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        for (int cluster_id = start_cluster_id;
            ...
            // ── NoStg epilogue: TMEM → regs → GMEM directly ──
            // Each lane owns ONE row (my_row); per sub-iter, lane writes
            // 32 bf16 of OUT, 32 bf16 of FAC_left, 32 bf16 of FAC_gate
            // straight to GMEM.  Each store is 16 B int4 × 4 per output.
            const uint32_t taddr_row_base =
                taddr_T + (((uint32_t)(cta_rank * BLOCK_M + row_warp * 32)) << 16);
            const int gr           = off_m + my_row;             // global out row
            const int out_n_base   = bid_n_local * BN_HALF;      // OUT col base
            const int fac_n_base_l = off_n;                       // FAC left base
            const int fac_n_base_g = off_n + BN_HALF;             // FAC gate base
            ...
            for (int sub = 0; sub < BN_HALF; sub += LD_X) {
                ...
                // Direct GMEM stores from registers.  Stride-32 across
                // lanes (each lane writes its own row), int4 within a lane.
                #pragma unroll
                for (int j = 0; j < LD_X / 8; j++) {
                    *reinterpret_cast<int4*>(&OUT_ptr[(size_t)gr * N_HALF + out_n_base   + sub + j * 8]) =
                        *reinterpret_cast<const int4*>(&out_pack[j * 4]);
                    *reinterpret_cast<int4*>(&FAC_ptr[(size_t)gr * N      + fac_n_base_l + sub + j * 8]) =
                        *reinterpret_cast<const int4*>(&fl_pack[j * 4]);
                    *reinterpret_cast<int4*>(&FAC_ptr[(size_t)gr * N      + fac_n_base_g + sub + j * 8]) =
                        *reinterpret_cast<const int4*>(&fg_pack[j * 4]);
                }
            }

            // After all TMEM reads are issued AND drained (wait_ld inside
            // the loop drained the last), MMA can reuse this TMEM half.
            // No bar.sync needed: no cross-warp dataflow.
            if (lane == 0) {
...
// Path B + nostg: persistent-only.  Without epi staging the entire 228 KB
// SMEM budget goes to the K-ring; NS=7 (224 KB) fits comfortably.
#define MAKE_LAUNCHER_BNOSTG(BN_, BK_, NS_, GSM_)                                      \
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                \
__launch_bounds__(256, LB_MIN_BLOCKS)                                                  \
void matmul_save_factors_b_nostg_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(       \
...
#define MAKE_GSM_SET_BNOSTG(BN_, BK_, NS_)  \
    MAKE_LAUNCHER_BNOSTG(BN_, BK_, NS_, 1)  \
    MAKE_LAUNCHER_BNOSTG(BN_, BK_, NS_, 4)  \
    MAKE_LAUNCHER_BNOSTG(BN_, BK_, NS_, 8)  \
    MAKE_LAUNCHER_BNOSTG(BN_, BK_, NS_, 16) \
    MAKE_LAUNCHER_BNOSTG(BN_, BK_, NS_, 32)

MAKE_GSM_SET_BNOSTG(256, 64, 4)
MAKE_GSM_SET_BNOSTG(256, 64, 7)

#undef MAKE_GSM_SET_BNOSTG
#undef MAKE_LAUNCHER_BNOSTG
