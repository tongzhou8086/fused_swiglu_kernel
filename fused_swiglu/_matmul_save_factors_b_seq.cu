    ...
    if (warp_id == 0 && elect_sync()) {
        ...
        for (int cluster_id = start_cluster_id;
            ...
            // ── Phase 2: SMEM → GMEM coalesced (4 warps cooperate) ──
            {
                ...
                constexpr int TB_SIZE    = NUM_EPI_WARPS * WARP_SIZE;     // 128
                {
                    ...
                    for (int s = 0; s < STORES_PER_THREAD; s++) {
                        const int flat = epi_tid + s * TB_SIZE;
...
#define MAKE_LAUNCHER_B(BN_, BK_, NS_, GSM_)                                          \
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                \
__launch_bounds__(256, LB_MIN_BLOCKS)                                                  \
void matmul_save_factors_b_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(             \
...
#define MAKE_GSM_SET_B(BN_, BK_, NS_)  \
    MAKE_LAUNCHER_B(BN_, BK_, NS_, 1)  \
    MAKE_LAUNCHER_B(BN_, BK_, NS_, 4)  \
    MAKE_LAUNCHER_B(BN_, BK_, NS_, 8)  \
    MAKE_LAUNCHER_B(BN_, BK_, NS_, 16) \
    MAKE_LAUNCHER_B(BN_, BK_, NS_, 32)

MAKE_GSM_SET_B(256, 64, 4)

#undef MAKE_GSM_SET_B
#undef MAKE_LAUNCHER_B
