"""CUDA launcher for the tanh-form-sigmoid save_factors variant (`_tanh`).

Replaces the divide-form sigmoid (`__fdividef(1, 1+__expf(-g))`) with
the tanh-form `0.5 + 0.5*tanh(g/2)` — one SFU op (MUFU.TANH) per
element instead of two (MUFU.EX2 + MUFU.RCP).
"""
import os
import numpy as np
import torch
...
_CU_PATH = os.path.join(_HERE, "_matmul_save_factors_tanh.cu")
_CUBIN   = os.path.join(_HERE, f"_matmul_save_factors_tanh_{SM_ARCH}.cubin")
...
def _kname(bn, bk, ns, gsm, persistent: bool = False):
    ...
    return f"matmul_save_factors_tanh{pers}_bm{BM}_bn{bn}_bk{bk}_ns{ns}_gsm{gsm}"
...
def matmul_save_factors_tanh(A, W_packed, config=_DEFAULT_CONFIG, persistent: bool = False):
    bn, bk, ns, gsm = config
    M, K = A.shape
    _, twoN = W_packed.shape
    assert twoN % 2 == 0
