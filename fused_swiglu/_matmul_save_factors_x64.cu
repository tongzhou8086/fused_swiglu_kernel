...
//     of `x32` (32 fp32/lane).
//   - With BN_HALF=128 and COL_PER_WARP=64, each warp now needs ONE load
//     each for left and gate (vs TWO loads each in the x32 version).
...
__device__ __forceinline__ void matmul_save_factors_impl(
    ...
    constexpr int LD_X          = 64;      // x64 variant: 64 b32 / lane per call
...
#define LOAD_TILE(slot_, k0_)                                                          \
    ...
    // Phase 1: all 8 warps read TMEM (4 row × 2 col split), fuse, stage to SMEM.
    {
        ...
        for (int sub = 0; sub < COL_PER_WARP; sub += LD_X) {
            ...
            tcgen05_ld_32x32b_x64(taddr_row_base + (uint32_t)n_left, left_vals);
            // Read gate chunk: TMEM cols [BN_HALF+n_left, BN_HALF+n_left+LD_X).
            float gate_vals[LD_X];
            tcgen05_ld_32x32b_x64(taddr_row_base + (uint32_t)(BN_HALF + n_left), gate_vals);
            tcgen05_wait_ld();
...
void matmul_save_factors_x64_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(           \
...
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                \
__launch_bounds__(256, LB_MIN_BLOCKS)                                                  \
void matmul_save_factors_x64_pers_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(      \
    const __grid_constant__ CUtensorMap A_tmap,                                        \
    const __grid_constant__ CUtensorMap B_tmap,                                        \
    __nv_bfloat16* OUT_ptr, __nv_bfloat16* FAC_ptr,                                    \
    int M, int N_HALF, int K)                                                          \
