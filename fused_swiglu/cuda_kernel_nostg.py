"""CUDA launcher for the NO-SMEM-staging save_factors variant (`_nostg`).

Epilogue writes directly from TMEM → registers → GMEM with no SMEM
staging buffer.  GMEM writes are uncoalesced across warp lanes (each
lane owns a different row).
"""
import os
import numpy as np
import torch
...
_CU_PATH = os.path.join(_HERE, "_matmul_save_factors_nostg.cu")
_CUBIN   = os.path.join(_HERE, f"_matmul_save_factors_nostg_{SM_ARCH}.cubin")
...
def _kname(bn, bk, ns, gsm, persistent: bool = False):
    ...
    return f"matmul_save_factors_nostg{pers}_bm{BM}_bn{bn}_bk{bk}_ns{ns}_gsm{gsm}"


def _smem_bytes(bn, bk, ns):
    """K-loop ring only (no SMEM staging in the epilogue)."""
...
def matmul_save_factors_nostg(A, W_packed, config=_DEFAULT_CONFIG, persistent: bool = False):
    bn, bk, ns, gsm = config
    M, K = A.shape
    _, twoN = W_packed.shape
    assert twoN % 2 == 0
