"""CUDA launcher for the WIDER-TMEM-LOAD save_factors variant (`_x64`).

Same architecture as cuda_kernel.py (persistent grid, b42-based main loop,
int4 SMEM→GMEM stores), but Phase 1 epilogue uses `tcgen05.ld.32x32b.x64`
instead of `x32` — one TMEM read per warp per output half (left/gate),
saving one `tcgen05.wait_ld` stall.
"""
import os
import numpy as np
import torch
...
_CU_PATH = os.path.join(_HERE, "_matmul_save_factors_x64.cu")
_CUBIN   = os.path.join(_HERE, f"_matmul_save_factors_x64_{SM_ARCH}.cubin")
...
_DEFAULT_CONFIG = (256, 64, 4, 16)        # NS=4 GSM=16 was the best int4-stores persistent config
...
def _kname(bn, bk, ns, gsm, persistent: bool = False):
    ...
    return f"matmul_save_factors_x64{pers}_bm{BM}_bn{bn}_bk{bk}_ns{ns}_gsm{gsm}"


def _smem_bytes(bn, bk, ns):
    """K-loop ring (epilogue aliases on top)."""
...
def matmul_save_factors_x64(A, W_packed, config=_DEFAULT_CONFIG, persistent: bool = False):
    """save_factors variant with x64 TMEM loads. Same signature as the
    baseline `matmul_save_factors`."""
    bn, bk, ns, gsm = config
    M, K = A.shape
    _, twoN = W_packed.shape
    assert twoN % 2 == 0
    ...
    grid_x = NUM_SMS if persistent else (M_pad // BM) * (twoN // bn)
    grid  = (grid_x, 1, 1)
    block = (NW * 32, 1, 1)
    fn(A_tmap, B_tmap,
