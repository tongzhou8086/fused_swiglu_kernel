// matmul_save_factors_b: Path B — K-loop / epilogue OVERLAP via double-buffered TMEM.
//
// Same fused SwiGLU semantics as _matmul_save_factors.cu, but the
// kernel is restructured so that tile T+1's K-loop runs IN PARALLEL
// with tile T's epilogue (Phase 1 + Phase 2).
//
// ── Architecture ─────────────────────────────────────────────────
//
//  TMEM: tcgen05_alloc_g2(2 * BLOCK_N) — full 512 cols used.
//    taddr_A = base + 0,         taddr_B = base + BLOCK_N.
//    Tile T's accumulator lives in taddr_{T & 1}.
//
//  Warp roles (PERMANENT across all tiles):
//    Warp 0 (BOTH CTAs)   : TMA — streams (A, W) tiles into the K-ring
//                            continuously, NEVER waits on the epilogue.
//    Warp 1 (CTA 0 only)  : MMA — runs K-loops back-to-back across tiles,
//                            writing to alternating TMEM halves.  Before
//                            REUSING a half (tile T >= 2), waits on
//                            epi_done_per_tile[T & 1] from the epilogue.
//    Warps 4..7 (BOTH CTAs): EPILOGUE workers — per tile, wait on
//                            mma_done_per_tile[T & 1], do Phase 1 (TMEM→
//                            SMEM) and Phase 2 (SMEM→GMEM), then arrive
//                            on epi_done_per_tile[T & 1] (cluster-multicast
//                            to CTA 0's mbar).
//    Warps 2..3            : IDLE.  (We could use them for extra epilogue
//                            parallelism, but symmetry across CTAs + clean
//                            row partitioning is easier with 4-warp epi.)
//
//  Mbarriers (in addition to the K-loop tile_ready/mma_done ring):
//    mma_done_per_tile[2]  : count=1, multicast-armed by MMA.  Fires
//                            both CTAs' local mbar when CTA 0's MMA
//                            finishes tile T's K-loop.  Epilogue waits.
//    epi_done_per_tile[2]  : count = NUM_EPI_WARPS * CTA_GROUP = 8.
//                            All epi warps on both CTAs arrive on CTA 0's
//                            mbar after consuming TMEM half {T & 1}.
//                            MMA waits before reusing that half.
//
//  SMEM layout (4 + 1 + 1 buffers):
//    [0,         RING_BYTES)             : K-loop ring (NS * (A + B) slots)
//    [RING_BYTES, +OUT_SH)               : OUT_sh staging
//    [+OUT_SH,   +FAC_SH)                : FAC_sh staging
//  At NS=4: ring=128KB, OUT_sh=~34KB, FAC_sh=~66KB.  Total ~228KB.  Fits.
//
// ── Inherited from _matmul_save_factors.cu ──────────────────────
//
...
// Async TMA STORE (SMEM → GMEM) via the bulk-copy engine.  Fire-and-forget:
// commit_group + wait_group track completion.  Cross-proxy fence is required
// between regular SMEM writes (generic proxy) and these reads (async proxy).
...
__device__ __forceinline__ void matmul_save_factors_impl(
    ...
    constexpr int BN_HALF        = BLOCK_N / 2;             // 128 at BN=256
    constexpr int MMA_K          = 16;
    constexpr int NUM_WARPS      = 8;
    constexpr int NUM_EPI_WARPS  = 4;       // warps 4..7 do the epilogue
    constexpr int EPI_WARP_FIRST = 4;
    constexpr int NUM_EPI_THREADS = NUM_EPI_WARPS * WARP_SIZE;   // 128 per CTA
    constexpr int LD_X           = 32;
    ...
    // ── SMEM layout: ring + DEDICATED staging (Path B) ──────────────────
    //   [0,         RING_BYTES)             : K-loop ring (NS × (A + B))
    //   [RING_BYTES, +OUT_SH_BYTES)         : OUT_sh staging
    //   [+OUT_SH_BYTES, +FAC_SH_BYTES)      : FAC_sh staging
    // Staging is NOT aliased on the ring (TMA can load NEXT tile while
    // epilogue is still reading the staging buffers).
    ...
    // No +8 padding: we are at the 228 KB SMEM cap and need the bytes.
    // Bank conflicts on int4 SMEM stores may show up but are dwarfed by
    // the K-loop / epilogue overlap saving we're chasing.
    constexpr int OUT_PAD      = BN_HALF;
    constexpr int FAC_PAD      = BLOCK_N;
    constexpr int OUT_SH_BYTES = BLOCK_M * OUT_PAD * sizeof(__nv_bfloat16);
    ...
    auto OUT_sh = reinterpret_cast<__nv_bfloat16 (*)[OUT_PAD]>(smem + RING_BYTES);
    auto FAC_sh = reinterpret_cast<__nv_bfloat16 (*)[FAC_PAD]>(smem + RING_BYTES + OUT_SH_BYTES);

    __shared__ uint64_t tile_ready_mbar[NUM_STAGES];
    __shared__ uint64_t mma_done_mbar[NUM_STAGES];
    __shared__ uint64_t mma_done_per_tile_mbar[2];   // MMA→Epi, count=1, multicast
    __shared__ uint64_t epi_done_per_tile_mbar[2];   // Epi→MMA, count=NUM_EPI_WARPS*CTA_GROUP
    __shared__ uint32_t tmem_addr_holder[1];

    constexpr int EPI_DONE_COUNT = NUM_EPI_WARPS * CTA_GROUP;   // = 8

    // ── One-time setup ──────────────────────────────────────────────────
    if (warp_id == 0 && elect_sync()) {
        #pragma unroll
        for (int s = 0; s < NUM_STAGES; s++) {
            mbarrier_init((uint32_t)__cvta_generic_to_shared(&tile_ready_mbar[s]), CTA_GROUP);
            mbarrier_init((uint32_t)__cvta_generic_to_shared(&mma_done_mbar[s]), 1);
        }
        mbarrier_init((uint32_t)__cvta_generic_to_shared(&mma_done_per_tile_mbar[0]), 1);
        mbarrier_init((uint32_t)__cvta_generic_to_shared(&mma_done_per_tile_mbar[1]), 1);
        mbarrier_init((uint32_t)__cvta_generic_to_shared(&epi_done_per_tile_mbar[0]),
                      EPI_DONE_COUNT);
        mbarrier_init((uint32_t)__cvta_generic_to_shared(&epi_done_per_tile_mbar[1]),
                      EPI_DONE_COUNT);
        // Pre-arrive mma_done[NS-1] so tile 0's TMA prologue can start
        // without waiting on MMA.  Subsequent tiles' phases continue
        // naturally because num_k_iters / NS is even (NS=4 at K=3584).
        asm volatile("mbarrier.arrive.shared::cta.b64 _, [%0];"
                     :: "r"((uint32_t)__cvta_generic_to_shared(&mma_done_mbar[NUM_STAGES - 1]))
                     : "memory");
        asm volatile("fence.mbarrier_init.release.cluster;");
    } else if (warp_id == 1) {
        // Double TMEM: 2 * BLOCK_N cols = full 512 cols.  Tile T's
        // accumulator lives in TMEM cols [(T&1)*BN, (T&1)*BN+BN).
    ...
    const uint32_t taddr_base = tmem_addr_holder[0];
    const uint32_t taddr_A    = taddr_base;
    const uint32_t taddr_B    = taddr_base + BLOCK_N;
    ...
    // ── LOAD_TILE ───────────────────────────────────────────────────────
    // Takes the slot, the K offset, AND the per-tile off_m / off_n_local
    // (since each tile in the persistent loop has different geometry).
    // This macro is used only by the TMA warp.

#define LOAD_TILE(slot_, k0_, off_m_, off_n_local_)                                    \
    do {                                                                                \
    ...
    // ── Path B: three independent warp loops ───────────────────────────
    //
    // Per-tile offset computation is repeated in each warp branch; the
    // logic is small and inlined here so we don't pay a function call.

#define COMPUTE_TILE_OFFSETS(cid_, off_m_out, off_n_out, off_n_local_out, bid_n_out)    \
    do {                                                                                \
        const int _num_in_group = GROUP_SIZE_M * grid_n;                                \
        const int _group_id     = (cid_) / _num_in_group;                               \
        const int _first_cm     = _group_id * GROUP_SIZE_M;                             \
        const int _gsm          = min(num_cluster_m - _first_cm, GROUP_SIZE_M);         \
        const int _cluster_m    = _first_cm + ((cid_) % _gsm);                          \
        const int _cluster_n    = ((cid_) % _num_in_group) / _gsm;                      \
        const int _bid_m        = _cluster_m * GROUP_M_INTRA + which_in_cluster;        \
        (bid_n_out)             = _cluster_n;                                           \
        (off_m_out)             = _bid_m * BLOCK_M;                                     \
        (off_n_out)             = (bid_n_out) * BLOCK_N;                                \
        (off_n_local_out)       = (off_n_out) + cta_rank * BLOCK_N_LOCAL;               \
    } while (0)

    if (warp_id == 0 && elect_sync()) {
        // ── TMA warp (BOTH CTAs) — continuous across all tiles ────
        uint32_t mma_done_phase[NUM_STAGES] = {};
        int global_load_idx = 0;
        for (int cluster_id = start_cluster_id;
             cluster_id < num_cluster_tiles;
             cluster_id += cluster_stride) {
            int off_m, off_n, off_n_local, bid_n_local;
            COMPUTE_TILE_OFFSETS(cluster_id, off_m, off_n, off_n_local, bid_n_local);
            (void)off_n; (void)bid_n_local;
            for (int k = 0; k < num_tiles_k; k++) {
                const int slot = global_load_idx % NUM_STAGES;
                if (global_load_idx >= NUM_STAGES - 1) {
                    const uint32_t mb =
                        (uint32_t)__cvta_generic_to_shared(&mma_done_mbar[slot]);
                    mbarrier_wait(mb, mma_done_phase[slot]);
                    mma_done_phase[slot] ^= 1;
                }
                LOAD_TILE(slot, k * BLOCK_K, off_m, off_n_local);
                global_load_idx++;
            }
        }
    } else if (cta_rank == 0 && warp_id == 1 && elect_sync()) {
        // ── MMA warp (CTA 0 only) — continuous, alternating TMEM halves ──
        uint32_t tile_ready_phase[NUM_STAGES] = {};
        uint32_t epi_done_phase[2] = {};
        int global_mma_idx = 0;
        int tile_idx = 0;
        for (int cluster_id = start_cluster_id;
             cluster_id < num_cluster_tiles;
             cluster_id += cluster_stride) {
            int off_m, off_n, off_n_local, bid_n_local;
            COMPUTE_TILE_OFFSETS(cluster_id, off_m, off_n, off_n_local, bid_n_local);
            (void)off_m; (void)off_n; (void)off_n_local; (void)bid_n_local;

            const int half = tile_idx & 1;
            const uint32_t taddr_T = (half == 0) ? taddr_A : taddr_B;

            // Wait for the previous tile that used this TMEM half to be
            // done before reusing it.  Only relevant once we wrap (>= 2).
            if (tile_idx >= 2) {
                const uint32_t mb =
                    (uint32_t)__cvta_generic_to_shared(&epi_done_per_tile_mbar[half]);
                mbarrier_wait(mb, epi_done_phase[half]);
                epi_done_phase[half] ^= 1;
            }

            for (int k = 0; k < num_tiles_k; k++) {
                const int slot = global_mma_idx % NUM_STAGES;
                const uint32_t mb =
                ...
                #pragma unroll
                for (int _kk = 0; _kk < K_MMAS; _kk++) {
                    // Reset accum on the first MMA of EACH tile (k==0).
                    const bool _accumulate = (k != 0) || (_kk > 0);
                    tcgen05_mma_g2(taddr_T, a_desc[_kk], b_desc[_kk], idesc, _accumulate);
                }
            ...
            // Signal that THIS tile's K-loop is complete; epi warps can
            // start reading TMEM half {half}.  Multicast → fires both
            // CTAs' local mma_done_per_tile mbar.
            tcgen05_commit_mcast_g2(
                (uint32_t)__cvta_generic_to_shared(&mma_done_per_tile_mbar[half]),
                cta_mask);
            tile_idx++;
        }
    } else if (warp_id >= EPI_WARP_FIRST && warp_id < EPI_WARP_FIRST + NUM_EPI_WARPS) {
        // ── Epilogue warps (warps 4..7 on BOTH CTAs) ──────────────
        uint32_t mma_done_per_tile_phase[2] = {};
        int tile_idx = 0;

        const int row_warp = warp_id - EPI_WARP_FIRST;   // 0..3
        const int lane     = tid % WARP_SIZE;
        const int my_row   = row_warp * 32 + lane;
        const int epi_tid  = row_warp * WARP_SIZE + lane;
        ...
        for (int cluster_id = start_cluster_id;
            ...
            // Wait for MMA to finish this tile's K-loop.
            {
                const uint32_t mb =
                    (uint32_t)__cvta_generic_to_shared(&mma_done_per_tile_mbar[half]);
                mbarrier_wait(mb, mma_done_per_tile_phase[half]);
                mma_done_per_tile_phase[half] ^= 1;
            }
            tcgen05_fence_after_thread_sync();

            // ── Phase 1: TMEM → SMEM staging ──
            // 4 row-warps × full BN cols, LD_X=32-wide passes over BN_HALF
            // (reading left+gate halves each iter).  4 iters per warp.
            const uint32_t taddr_row_base =
                taddr_T + (((uint32_t)(cta_rank * BLOCK_M + row_warp * 32)) << 16);
            #pragma unroll
            for (int sub = 0; sub < BN_HALF; sub += LD_X) {
                ...
                __nv_bfloat162 fl_pack [LD_X / 2];
                __nv_bfloat162 fg_pack [LD_X / 2];
                #pragma unroll
            ...
            // SMEM staging before any reads it (incl. async-proxy reads).
            asm volatile("bar.sync 1, %0;" :: "n"(NUM_EPI_THREADS));

            // Cross-proxy fence: Phase 1 wrote SMEM via the generic proxy
            // (regular st.shared via int4).  The TMA store engine reads
            // SMEM via the async proxy.  Without this fence, async-proxy
            // reads may observe stale or partially-written bytes.
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");

            // ── EARLY epi_done arrive ──
            // TMEM half is no longer accessed by this warp after Phase 1.
            // Signal MMA NOW so it can start writing tile T+2 to this TMEM
            // half while the TMA stores for tile T drain in the background.
            if (lane == 0) {
            ...
            // Two stores per tile (issued by epi warp 0, single lane):
            //   - OUT: [BM, BN_HALF] @ (out_n_base, off_m)
            //   - FAC: [BM, BLOCK_N] @ (off_n, off_m)
            // Fire-and-forget; commit_group tracks completion.
            if (warp_id == EPI_WARP_FIRST && lane == 0) {
                const int out_n_base = bid_n_local * BN_HALF;
            ...
            // End-of-tile drain.  OUT_sh / FAC_sh live OUTSIDE the ring
            // (non-overlapping) but the NEXT tile's Phase 1 overwrites the
            // SAME staging bytes, so we must wait for this tile's TMA reads
            // of SMEM to complete before proceeding.
            // wait_group.read 0 is per-warp and a no-op for warps that
            // didn't commit; the bar.sync gates the committing warp.
            tma_wait_group<0>();
            asm volatile("bar.sync 1, %0;" :: "n"(NUM_EPI_THREADS));
            tile_idx++;
        }
    }
    // Warps 2 and 3 fall through with no work — they wait at the cluster
    // barrier below.

#undef LOAD_TILE
#undef COMPUTE_TILE_OFFSETS

    // ── Cleanup: cluster-wide sync, then TMEM dealloc ──────────────────
    // All warps reach the cluster barrier after their respective loops
    // complete.  This serializes the final TMEM read by any straggling
    // epi warp before warp 0 deallocates TMEM.
    asm volatile("barrier.cluster.arrive.release.aligned;");
    asm volatile("barrier.cluster.wait.acquire.aligned;");

    if (warp_id == 0 && elect_sync()) {
        tcgen05_dealloc_g2(taddr_base, 2 * BLOCK_N);
    }
}

#if 0
// ── DEAD CODE removed ── (the original cluster_id-wrapping implementation
//                          lives below this block intentionally; #if 0'd out)
    if (warp_id == 0 && elect_sync()) {
...
// Path B is persistent-only (overlap doesn't apply with 1 tile per CTA),
// and NS=4 only (NS=7 ring + 100 KB staging = 324 KB > 228 KB cap).
#define MAKE_LAUNCHER_BTMAST(BN_, BK_, NS_, GSM_)                                      \
extern "C" __global__ __cluster_dims__(CTA_GROUP, 1, 1)                                \
__launch_bounds__(256, LB_MIN_BLOCKS)                                                  \
void matmul_save_factors_b_tmast_bm128_bn##BN_##_bk##BK_##_ns##NS_##_gsm##GSM_(       \
...
#define MAKE_GSM_SET_BTMAST(BN_, BK_, NS_)  \
    MAKE_LAUNCHER_BTMAST(BN_, BK_, NS_, 1)  \
    MAKE_LAUNCHER_BTMAST(BN_, BK_, NS_, 4)  \
    MAKE_LAUNCHER_BTMAST(BN_, BK_, NS_, 8)  \
    MAKE_LAUNCHER_BTMAST(BN_, BK_, NS_, 16) \
    MAKE_LAUNCHER_BTMAST(BN_, BK_, NS_, 32)

MAKE_GSM_SET_BTMAST(256, 64, 4)

#undef MAKE_GSM_SET_BTMAST
#undef MAKE_LAUNCHER_BTMAST
